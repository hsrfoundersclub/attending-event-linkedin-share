...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    // Check for restored form data in URL
    const restoredText = searchParams.get("text");
    const restoredImage = searchParams.get("image");

    if (restoredText) {
      setText(decodeURIComponent(restoredText));
    }

    if (restoredImage) {
      // Create a temporary div to hold the image
      const tempDiv = document.createElement("div");
      tempDiv.innerHTML = `<img src="${decodeURIComponent(
        restoredImage
      )}" alt="Restored image" />`;
      document.body.appendChild(tempDiv);
      // You'll need to implement your own image restoration logic here
